...
Run all five passes in order:

```bash
for n in 10 20 40 60 80; do
  uv run python ablation_full_dataset/scripts/run_batch_size.py --batch-size $n || break
done
```

After all passes finish, write the comparison tables and plot:
